...
public class LogoutResourceIT {
    ...
    public void before() throws Exception {
        ...
        claims.put("sub", 123);
        this.idToken = new OidcIdToken(TestUtil.ID_TOKEN, Instant.now(), Instant.now().plusSeconds(60), claims);

        SecurityContextHolder.getContext().setAuthentication(TestUtil.authenticationToken(idToken));
    ...
    public void getLogoutInformation() throws Exception {
        ...
        restLogoutMockMvc.perform(post("/api/logout"))
            ...
            .andExpect(jsonPath("$.idToken").value(TestUtil.ID_TOKEN));
    }
}
